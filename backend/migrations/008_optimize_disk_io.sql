-- ============================================================
-- ER&GG v2.0 - DISK IO 최적화
-- 1) SAVEPOINT 제거 (BEGIN...EXCEPTION → 사전 검증)
-- 2) v2_ / old 트랜잭션 분리 (단일 거대 트랜잭션 → 2개 소형 트랜잭션)
--
-- process_game_batch → process_game_v2 + process_game_old
-- Supabase Dashboard → SQL Editor에서 실행
-- ============================================================

-- ────────────────────────────────────────────────────────────
-- 1. process_game_v2: v2_ 테이블만 처리 (SAVEPOINT 없음)
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- 2. process_game_old: old 테이블만 처리 (forward only, SAVEPOINT 없음)
-- ────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION process_game_old(p_data JSONB)
RETURNS JSONB AS $$
DECLARE
  v_patch TEXT := p_data->>'patch_version';
  v_p JSONB;
  ...
  v_tier TEXT;
  v_tiers TEXT[];
  v_cn INT; v_bw INT; v_gr INT; v_pk INT; v_pa INT;
  v_eq0 INT; v_eq1 INT; v_eq2 INT; v_eq3 INT; v_eq4 INT;
  v_tfc INT; v_mb INT; v_ma INT;
  v_rp NUMERIC;
  v_fs1 INT; v_fs2 INT; v_fs3 INT; v_fs4 INT;
  v_ok INT := 0;
  v_fail INT := 0;
...
BEGIN
  ...
  LOOP
    ...
    v_fs1 := COALESCE((v_p->'fs'->0)::INT, 0);
    v_fs2 := COALESCE((v_p->'fs'->1)::INT, 0);
    v_fs3 := COALESCE((v_p->'fs'->2)::INT, 0);
    v_fs4 := COALESCE((v_p->'fs'->3)::INT, 0);
