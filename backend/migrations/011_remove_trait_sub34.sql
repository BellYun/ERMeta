-- ============================================================
-- v2_CharacterTraitBuildStats: sub3/sub4/optionTrait3/optionTrait4 제거
-- NULL 컬럼으로 인해 ON CONFLICT upsert가 실패하던 버그 수정
-- PostgreSQL UNIQUE 제약은 NULL != NULL이므로 COALESCE unique index 사용
-- ============================================================
...
ALTER TABLE "v2_CharacterTraitBuildStats"
  DROP CONSTRAINT IF EXISTS "v2_CharacterTraitBuildStats_characterNum_mainCore_sub1_sub2_key";

-- 혹시 다른 이름일 수 있으므로
DROP INDEX IF EXISTS "v2_CharacterTraitBuildStats_characterNum_mainCore_sub1_sub2_key";

-- 2. 컬럼 삭제
ALTER TABLE "v2_CharacterTraitBuildStats"
  DROP COLUMN IF EXISTS "sub3",
  DROP COLUMN IF EXISTS "sub4",
  DROP COLUMN IF EXISTS "optionTrait3",
  DROP COLUMN IF EXISTS "optionTrait4";

-- 3. 중복 row 합산 후 정리 (id 컬럼 없음 → ctid 사용)
-- Step A: 합산된 데이터를 임시 테이블에 저장
CREATE TEMP TABLE _trait_merged AS
SELECT
  "characterNum",
  MIN("mainCore") AS "mainCore",
  MIN("sub1") AS "sub1",
  MIN("sub2") AS "sub2",
  MIN("optionTrait1") AS "optionTrait1",
  MIN("optionTrait2") AS "optionTrait2",
  MIN("bestWeapon") AS "bestWeapon",
  "tier",
  "patchVersion",
  SUM("totalGames") AS "totalGames",
  SUM("totalWins") AS "totalWins",
  SUM(COALESCE("totalRP", 0)) AS "totalRP"
...
GROUP BY
  ...
  COALESCE("sub1", 0),
  COALESCE("sub2", 0),
  COALESCE("optionTrait1", 0),
  COALESCE("optionTrait2", 0),
  COALESCE("bestWeapon", 0),
  "tier",
  "patchVersion";

-- Step B: 기존 데이터 전부 삭제
TRUNCATE "v2_CharacterTraitBuildStats";

-- Step C: 합산 데이터 재삽입
INSERT INTO "v2_CharacterTraitBuildStats" (
  "characterNum", "mainCore", "sub1", "sub2",
...
DROP TABLE _trait_merged;

-- 4. COALESCE unique index (NULL-safe upsert)
CREATE UNIQUE INDEX IF NOT EXISTS idx_v2_trait_build_upsert
ON "v2_CharacterTraitBuildStats" (
  "characterNum",
  COALESCE("mainCore", 0),
  COALESCE("sub1", 0),
  COALESCE("sub2", 0),
...
BEGIN
  INSERT INTO "v2_CharacterTraitBuildStats" (
    ...
    p_character_num, p_main_core, p_sub1, p_sub2,
    p_option1, p_option2,
...
-- 5. process_game_batch 내 호출 부분 업데이트
-- process_game_batch 전체를 재정의 (sub3/sub4 파라미터 제거)
...
DECLARE
  ...
  -- trait sub arrays (only first 2 elements each)
  v_fs1 INT; v_fs2 INT;
  v_ss1 INT; v_ss2 INT;
  v_skill_order INT[];
  -- 결과 추적
...
BEGIN
  ...
  LOOP
    BEGIN -- ← 참가자별 에러 격리
      ...
      -- trait subs (only first 2 of each)
      ...
      LOOP
        ...
        -- v2_CharacterTraitBuildStats (sub3/sub4 제거됨)
        IF v_tfc > 0 THEN
        ...
        IF v_is_forward THEN
          ...
          IF v_tfc > 0 THEN
            PERFORM upsert_old_character_trait_build(
              v_cn, v_tfc,
              COALESCE(v_fs1, 0), COALESCE(v_fs2, 0), 0, 0,
              v_bw, v_tier, v_patch, 1,
              CASE WHEN v_gr = 1 THEN 1 ELSE 0 END,
              CASE WHEN v_gr <= 3 THEN 1 ELSE 0 END,
              v_rp::INT, v_gr,
