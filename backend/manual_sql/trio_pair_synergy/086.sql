-- ============================================================
-- character 86 (펜리르) - 페어 통계 + 솔로 baseline
-- patch 10.x, diamond+
--
-- 사용법:
--   1) 아래 두 쿼리를 Supabase SQL Editor 에 한 번에 붙여넣고 Run
--   2) 첫 번째 결과 (pairs) → 086.json 의 "pairs" 배열에 paste
--   3) 두 번째 결과 (solo) → 086.json 의 "soloBaseline" 배열에 paste
-- ============================================================
...
WITH focus_partners AS (
  SELECT ally1_char AS focus_char, ally1_weapon AS focus_weapon,
   ...
   WHERE ally1_char = 86 AND ally2_char NOT IN (9998, 9999)
  UNION ALL
  SELECT ally1_char, ally1_weapon, third_char, third_weapon,
         total_games, total_wins, total_rp, rank_sum
    FROM "v2_CharacterTrioWeaponSearch_p10"
   WHERE ally1_char = 86 AND third_char NOT IN (9998, 9999)
  UNION ALL
  SELECT ally2_char, ally2_weapon, ally1_char, ally1_weapon,
         total_games, total_wins, total_rp, rank_sum
    FROM "v2_CharacterTrioWeaponSearch_p10"
   WHERE ally2_char = 86 AND ally1_char NOT IN (9998, 9999)
  UNION ALL
  SELECT ally2_char, ally2_weapon, third_char, third_weapon,
         total_games, total_wins, total_rp, rank_sum
    FROM "v2_CharacterTrioWeaponSearch_p10"
   WHERE ally2_char = 86 AND third_char NOT IN (9998, 9999)
  UNION ALL
  SELECT third_char, third_weapon, ally1_char, ally1_weapon,
         total_games, total_wins, total_rp, rank_sum
    FROM "v2_CharacterTrioWeaponSearch_p10"
   WHERE third_char = 86 AND ally1_char NOT IN (9998, 9999)
  UNION ALL
  SELECT third_char, third_weapon, ally2_char, ally2_weapon,
         total_games, total_wins, total_rp, rank_sum
    FROM "v2_CharacterTrioWeaponSearch_p10"
   WHERE third_char = 86 AND ally2_char NOT IN (9998, 9999)
)
SELECT
  focus_weapon,
  partner_char,
