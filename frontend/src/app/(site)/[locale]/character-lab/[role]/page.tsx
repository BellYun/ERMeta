...
export default async function CharacterLabRolePage({ params, searchParams }: Props) {
    ...
    <main className="page-shell mx-auto max-w-6xl px-3 py-6 sm:px-5 sm:py-8">
      ...
      <header className="dashboard-panel mb-5 flex flex-col gap-3 p-4">
            ...
            {[
              {
                ...
                href: `/${locale}/character-lab/${supportedRole}`,
              },
              {
                label: "2. 입장료 보정",
                description: "티어 차이 반영",
                active: isEntryAdjusted,
                href: `/${locale}/character-lab/${supportedRole}?metric=entry`,
              },
              {
                label: "3. 판수 신뢰 보정",
                description: "상승폭 × √판수",
                active: isSampleConfidence,
                href: `/${locale}/character-lab/${supportedRole}?metric=sample`,
              },
              {
                label: "4. 입장료+판수 보정",
                description: "두 보정 동시 적용",
                active: isCombinedConfidence,
                href: `/${locale}/character-lab/${supportedRole}?metric=combined`,
              },
            ].map((item) => (
              <Link
                key={item.label}
        ...
        {locale === "ko" && isSampleConfidence ? (
          <p className="border-l-2 border-[var(--color-accent)] pl-3 text-xs leading-5 text-[var(--color-muted-foreground)]">
            캐릭터별 대표 파트너 조합을 관측 상승폭 × √판수로 다시 고른 뒤, 파트너 역할군과 내부
