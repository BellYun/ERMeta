...
interface LegacyCharacterLabPageProps {
...
export async function LegacyCharacterLabPage({ params }: LegacyCharacterLabPageProps) {
  const { locale } = await params;
  if (!isRouteLocale(locale)) notFound();
  setRequestLocale(locale);
  const copy = COPY[locale];
  const roleLabels = ROLE_LABELS[locale];
  const totalCharacters = ROLES.reduce((sum, role) => sum + role.data.characters.length, 0);
  const totalGroups = ROLES.reduce((sum, role) => sum + role.data.groupK, 0);
