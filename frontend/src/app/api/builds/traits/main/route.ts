...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const [mainGroup, rows] of mainMap) {
      ...
      const ALL_GROUPS: TraitGroup[] = (
        ["havoc", "fortification", "support", "chaos"] as TraitGroup[]
      ).filter((g) => g !== mainGroup);
      ...
      builds.push({
        ...
        mainCoreOptions: aggregateOptions(rows, "mainCore", mainTotal, {
          allCodes: TRAIT_CORES[mainGroup],
        }),
        sub1Options: aggregateOptions(rows, ["sub1", "sub2"], mainTotal, {
          allCodes: TRAIT_SUBS_SLOT1[mainGroup],
        }),
        sub2Options: aggregateOptions(rows, ["sub1", "sub2"], mainTotal, {
