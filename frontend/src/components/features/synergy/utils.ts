...
export function getSortValue(rec: TrioResult, sortBy: SortBy): number {
  if (sortBy === "averageRP") return rec.averageRP;
  if (sortBy === "winRate") return rec.winRate;
  if (sortBy === "averageRank") return -rec.averageRank;
  return rec.totalGames;
}

export function getThirdCharacter(rec: TrioResult, allyA: number, allyB: number): number | null {
...
export function deduplicateResults(
  ...
  if (selectedAllies.length === 2) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (!existing || getSortValue(rec, sortBy) > getSortValue(existing.rec, sortBy)) {
        map.set(key, { rec, idx: i });
      }
    }
    return Array.from(map.values())
      .sort((a, b) => getSortValue(b.rec, sortBy) - getSortValue(a.rec, sortBy))
      .map((v) => v.rec);
  }
  ...
  if (selectedAllies.length === 1) {
    ...
    for (let i = 0; i < results.length; i++) {
      const rec = results[i];
      const others = [rec.character1, rec.character2, rec.character3].filter((c) => c !== selected);
